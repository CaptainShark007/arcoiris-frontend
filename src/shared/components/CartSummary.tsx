...
import { useUsers, useCustomer } from '@shared/hooks';
...
export const CartSummary = memo(({
  ...
  const { isLoading: isLoadingCustomer } = useCustomer(userId);
        ...
        return (
          <>
            <Button
              ...
              disabled={isProcessing || isLoadingCustomer}
            >
              {isProcessing ? 'Procesando...' : isLoadingCustomer ? 'Cargando datos...' : 'Confirmar orden'}
            </Button>
                  ...
                  <TableRow key={item.id} sx={{ '&:last-child td': { border: 0 } }}>
                    ...
                    <TableCell sx={{ borderBottom: 1, borderColor: 'divider' }}>
                      ...
                      <Typography variant='caption' color='text.secondary' sx={{ display: 'block', mb: 0.5 }}>
                        {[item.variant?.color, item.variant?.storage, item.variant?.finish].filter(Boolean).join(' • ')}
                      </Typography>
